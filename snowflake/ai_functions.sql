        ...
        messages => ARRAY_CONSTRUCT(
            OBJECT_CONSTRUCT(
                ...
                'content',
                 ...
                 || '\n- NO STATISTICS IN THE HEADLINE. No percentages, shares, ratios, averages '
                 || 'or counts — not as digits and not spelled out as words. "Fifty-six percent of '
                 || 'it happened after midnight" is a report, and nobody shares a report. Say what '
                 || 'the shape MEANS: "It only ever got built after the world went to sleep."'
                 || '\n- A number is allowed only when it names a MOMENT or a SILENCE, and only '
                 || 'one: a 3:53am ending, a 107-day gap. Those are events. A share is not.'
                 || '\n- The reader can already see the proportion — every commit is a dot on the '
                 || 'chart, and the header prints the count. Repeating it in words wastes the one '
                 || 'sentence that could have said something.'
